...
* 如果是按需引入，那么最后是按照需要进行打包，JS会被打包在引用的那个JS中了，CSS是分开打包。主要因为是同步引用。
```
1 需要下载个插件
...
* 遇到这么个问题，ec组件内有个JS需要引入`'./xx.hdr'`这种后缀的文件。不仅在打dev模式会出错，build也会出错。
* 我就想的这样：dev模式，src会被作为一个静态资源的服务器的根目录，所以测试时内部的img路径需要配置。bulid就需要自己手动挪过去这个`'./xx.hdr'`。
...
* 那么就可以理解这个module是做什么用，就是webpack在识别各个被引入模块的时候，需要做的编译处理或输出配置。
